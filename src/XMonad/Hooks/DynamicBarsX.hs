...
module XMonad.Hooks.DynamicBarsX (
  -- * Usage
  -- $usage
    DynamicStatusBar
...
import XMonad.Hooks.DynamicLogX
...
-- Make PP work with DynamicBars
singlePP :: PP -> X()
singlePP = singlePPFormat dynamicLogString

singlePPFormat :: (PP -> X String) -> PP -> X ()
singlePPFormat dynlStr pp = do
  (_, dsbInfoHandles) <- XS.get >>= return . unzip . dsbInfo
  singlePP' dynlStr pp dsbInfoHandles

singlePP' :: (PP -> X String) -> PP -> [Handle] -> X ()
singlePP' dynlStr pp handles = do
  st <- get
  let pickPP :: WorkspaceId -> WriterT (Last XState) X String
      pickPP ws = lift $ dynlStr $ pp
